...
async function getAuthUser(prisma: PrismaCtx, requestObj: any) {
  ...
  if (secret === "") {
    // zJED PROD TODO: Switch to return errRobj(ERR_01_NOT_AUTHORIZED)
    return errRobj(
      "ERROR:SERVER:prismaGetAuthUser:: Invalid server configuration. Secret missing.",
    );
  }
  try {
    ...
    // zJED PROD TODO: Switch to return errRobj(ERR_01_NOT_AUTHORIZED) and remove console.logs
    ...
    return errRobj("ERROR:SERVER:prismaGetAuthUser:: Request missing authentication headers");
  }
  try {
    ...
    if (
      ...
      // zJED PROD TODO: Switch to return errRobj(ERR_01_NOT_AUTHORIZED)
      return errRobj("ERRROR:SERVER:prismaGetAuthUser:: Token invalid");
    }
    ...
    if (!valid) {
      return errRobj("ERRROR:SERVER:prismaGetAuthUser:: Unexpected response when authenticating.");
    }
    ...
    if (user.data.deletedAt) {
      // zJED PROD TODO: Switch to return errRobj(ERR_01_NOT_AUTHORIZED)
      return errRobj("ERRROR:SERVER:prismaGetAuthUser:: User marked as deleted.");
    }
  } catch (err) {
